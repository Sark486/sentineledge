...
def test_the_telemetry_service_holds_no_session_of_its_own():
    """It never commits, so holding a session would only invite it to."""
    service = create_telemetry_service(as_session(FakeSession()))
...
def test_each_call_builds_a_fresh_service():
    ...
    assert create_telemetry_service(as_session(session)) is not create_telemetry_service(
        as_session(session)
    )
    assert create_device_service(as_session(session)) is not create_device_service(
        as_session(session)
    )
...
async def test_deps_build_services_over_the_request_session():
    ...
    telemetry = await get_telemetry_service(as_session(session))
    assert telemetry.repo.session is session
    assert (await get_device_service(as_session(session))).session is session
