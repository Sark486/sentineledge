...
class Device(Base):
    ...
    # NOT NULL with a server default: a persisted device always has a last_seen,
    # which for a never-reporting device is its registration time.
...
class ClimateAggregateView(Base, ClimateDataMixin):
    ...
    timestamp: Mapped[datetime] = mapped_column(
        "bucket", DateTime(timezone=True), primary_key=True
    )

    # Redeclared to drop the mixin's ForeignKey - a view cannot carry one - and
    # to make device_id part of the composite key the aggregate groups by.
